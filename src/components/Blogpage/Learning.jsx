...
import { useState } from "react";
import { useParams, Link } from "react-router-dom";
import { FileCode, FileText, FileImage, Book, ChevronRight, FileBarChart2, Presentation, Menu, X } from "lucide-react";
import { useDeleteLearningMutation, useGetLearningsQuery } from "../../features/learningsApiSlice"; // <-- very important
import FileViewer from "./FileViewer";
import { useSelector } from "react-redux";
...
const LearningDetail = () => {
    ...
    if (isError || !learnings?.entities[topicId]) {
        return (
            <div className="min-h-screen bg-[#1e1e1e] text-white flex flex-col items-center justify-center px-4">
                <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-center">Topic Not Found</h1>
                <Link to="/learning" className="text-green-400 hover:underline">
                    Back to Learning
                </Link>
            </div>
    ...
    const handleFileSelect = (file) => {
        setActiveFile(topic.files.indexOf(file));
    ...
    return (
        <div className="min-h-screen bg-[#1e1e1e] text-white flex flex-col">
            {/* Breadcrumb */}
            <header className="border-b border-gray-700 py-3 sticky top-0 bg-[#1e1e1e] z-20">
                <div className="flex items-center px-4 gap-2 text-sm text-gray-400">
                    {/* Mobile menu button */}
                    <button
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto">
                        <Link to="/blogs" className="hover:underline whitespace-nowrap">Learning</Link>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                {/* Sidebar */}
                <div className={`
                    fixed lg:static inset-y-0 left-0 z-30 w-64 border-r border-gray-700 bg-[#252525] flex flex-col
